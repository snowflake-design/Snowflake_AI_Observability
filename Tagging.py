...
from streamlit_flow.elements import StreamlitFlowNode, StreamlitFlowEdge, StreamlitFlowState
...
class DocumentProcessor:
    ...
    def generate_tags_for_chunk(self, chunk: Dict[str, Any]) -> List[str]:
        ...
        prompt = f"""
        Analyze the following text chunk and generate 4-5 relevant tags that capture:
        1. Major topic/theme discussed
        2. Key entities mentioned
        3. Content type/category
        4. Any processes or actions described
        5. Emotional tone or sentiment if applicable
    ...
    if text_content and process_button:
        ...
        with tab1:
            ...
            if hierarchy and hierarchy.get('nodes'):
                ...
                selected_element = streamlit_flow(
                    'tag_hierarchy_flow',
                ...
                # Handle selection
                if selected_element and selected_element.get('type') == 'node':
                    node_data = selected_element['data']
                    ...
                    with col1:
                        st.markdown(f"**Tag:** {node_data['label']}")
                        st.markdown(f"**Level:** {node_data['level']}")
                        st.markdown(f"**Associated Chunks:** {node_data['chunk_count']}")
                        
                        if node_data.get('chunk_details'):
                            st.markdown("**Chunk Previews:**")
                            for chunk in node_data['chunk_details'][:3]:  # Show first 3 chunks
                                if chunk:
                                    preview = chunk['text'][:200] + "..." if len(chunk['text']) > 200 else chunk['text']
                                    st.markdown(f"""
                                    <div class="chunk-preview">
                                        <strong>{chunk['id']}:</strong> {preview}
                                    </div>
                                    """, unsafe_allow_html=True)
